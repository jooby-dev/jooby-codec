...
export const getGsmBlock = ( commandName: string, bytes: types.TBytes ): IGsmBlock => {
    ...
    if ( block.crc.calculated !== block.crc.received ) {
        throw new Error(`Command ${commandName}. Invalid block crc. Calculated: ${block.crc.calculated}, received: ${block.crc.received}`);
    }

    const [blockPrefix, blockLength] = block.payload;

    if ( blockPrefix !== GSM_BLOCK_PREFIX ) {
        throw new Error(`Command ${commandName}. Invalid block prefix: ${blockPrefix}.`);
    }

    if ( blockLength > GSM_BLOCK_SIZE ) {
        throw new Error(`Command ${commandName}. Invalid block length: ${blockLength}.`);
    }
    ...
    return {
        index,
        /* slice blockPrefix and blockLength */
        data: block.payload.slice(2, 2 + blockLength)
    };
};

export const setGsmBlock = ( block: IGsmBlock ): types.TBytes => {
    const data = [GSM_BLOCK_PREFIX, block.data.length, ...block.data];

    if ( data.length < GSM_BLOCK_SIZE ) {
        data.push(...new Array<types.TUint8>(GSM_BLOCK_SIZE - block.data.length).fill(0));
    }
